...
> یادداشتِ صادقانه دامنه: این turn روی شکایتِ اصلیِ تأییدشده (no-op بودنِ «ثبتِ عدمِ حضور») و ساختِ هارنسِ تستِ کلیکیِ واقعی متمرکز شد. سایر مواردِ فهرست‌شده (مودالِ تمام‌صفحه‌ی براکت، بازطراحیِ کاملِ چت، و…) در همین مخزن قابلِ ادامه‌اند و با همین هارنسِ Playwright می‌توان هرکدام را قبل از تحویل کلیک‌تست کرد.
